...
class LSM6DS3:
    def __init__(self, i2c, address=0x6A, mode=NORMAL_MODE_104HZ):
        ...
        self.bus.mem_write(bytearray([self.mode]), self.address, CTRL2_G)

        # Set accel mode/enable
        self.bus.mem_write(bytearray([self.mode]), self.address, CTRL1_XL )

        # Send the reset bit to clear the pedometer step count
        self.bus.mem_write(bytearray([RESET_STEPS]), self.address, CTRL10_C)

        # Enable sensor functions (Tap, Tilt, Significant Motion)
        self.bus.mem_write(bytearray([SET_FUNC_EN]), self.address, CTRL10_C)

        # Enable X Y Z Tap Detection
        self.bus.mem_write(bytearray([TAP_EN_XYZ]), self.address, TAP_CFG )

        # Enable Double tap
        self.bus.mem_write(bytearray([DOUBLE_TAP_EN]), self.address, WAKE_UP_THS)

        # Set tap threshold
        self.bus.mem_write( bytearray([TAP_THRESHOLD]), self.address, TAP_THS_6D)

        # Set double tap max time gap
        self.bus.mem_write(bytearray([DOUBLE_TAP_DUR]), self.address, INT_DUR2)

    def _read_reg(self, reg, size):
        return self.bus.mem_read(size, self.address, reg)
